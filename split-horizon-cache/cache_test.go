...
func TestCacheInsertion(t *testing.T) {
	...
	for _, tc := range cacheTestCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			valid, k := key(state.Name(), m, mt, state.Do(), state.Req.CheckingDisabled, c.bucketOf(state.IP()))
			...
			i := c.getIfNotStale(time.Now().UTC(), state, "dns://:53", c.bucketOf(state.IP()))
			found := i != nil
...
func TestCacheWildcardMetadata(t *testing.T) {
	...
	_, k := key(qname, w.Msg, response.NoError, state.Do(), state.Req.CheckingDisabled, c.bucketOf(state.IP()))
	i, _ := c.pcache.Get(k)
...
func TestCacheSeparation(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			item := c.getIfNotStale(time.Now().UTC(), state, "dns://:53", c.bucketOf(state.IP()))
			found := item != nil
...
func TestServfailDoesNotShadowPositiveCache(t *testing.T) {
	...
	req.SetQuestion("example.org.", dns.TypeA)
	state := request.Request{W: &test.ResponseWriter{}, Req: req}
	k := hash(state.Name(), state.QType(), state.QClass(), state.Do(), state.Req.CheckingDisabled, c.bucketOf(state.IP()))
	...
	got := c.getIfNotStale(now.Add(time.Second), state, "test", c.bucketOf(state.IP()))
	if got == nil {
...
func TestPreferPositiveCachePolicy(t *testing.T) {
	...
	if got := c.getIfNotStale(now, state, "test", c.bucketOf(state.IP())); got == nil || got.Rcode != dns.RcodeNameError {
		t.Fatalf("default policy should prefer ncache NXDOMAIN, got %+v", got)
	}

	c.preferPositive = true
	if got := c.getIfNotStale(now, state, "test", c.bucketOf(state.IP())); got == nil || got.Rcode != dns.RcodeSuccess {
...
func TestPreferPositiveRetainsLKGAcrossNonAnswerSuccessRefreshes(t *testing.T) {
	...
	req.SetQuestion("cached.org.", dns.TypeA)
	state := request.Request{W: &test.ResponseWriter{}, Req: req}
	writer := &ResponseWriter{Cache: c, state: state, prefetch: true, src: c.bucketOf(state.IP())}
	...
	for i, refresh := range refreshes {
		...
		got := c.getIfNotStale(now, state, "test", c.bucketOf(state.IP()))
