...
func (c *Cache) ServeDNS(ctx context.Context, w dns.ResponseWriter, r *dns.Msg) (int, error) {
	...
	i := c.getIfNotStale(now, state, server, src)
	if i == nil {
		...
		crr := &ResponseWriter{ResponseWriter: w, Cache: c, state: refreshState, server: server, do: do, ad: ad, cd: cd,
			nexcept: c.nexcept, pexcept: c.pexcept, wildcardFunc: wildcardFunc(ctx), src: src}
	...
	if stale {
		...
		if c.verifyStale {
			...
			if !trackRefresh || i.beginRefresh(now, failureRecheck) {
				refreshState := authenticatedRefreshState(state)
				crr := &ResponseWriter{ResponseWriter: w, Cache: c, state: refreshState, server: server, do: do, ad: ad, cd: cd, src: src}
...
func (c *Cache) doPrefetch(ctx context.Context, cw *ResponseWriter, i *item, now time.Time, stale bool) bool {
	...
	if i1 := c.exists(cw.state.Name(), cw.state.QType(), cw.state.QClass(), cw.do, cw.cd, cw.src); i1 != nil {
		i1.Reset(now, i.Hits())
	}
	return true
}
...
func (c *Cache) getIfNotStale(now time.Time, state request.Request, server string, src []byte) *item {
	k := hash(state.Name(), state.QType(), state.QClass(), state.Do(), state.Req.CheckingDisabled, src)
...
func (c *Cache) exists(name string, qtype, qclass uint16, do, cd bool, src []byte) *item {
	k := hash(name, qtype, qclass, do, cd, src)
	if i, ok := c.ncache.Get(k); ok {
		return i
	}
	if i, ok := c.pcache.Get(k); ok {
