...
func TestServeStaleFailureRecheckImmediate(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			item := c.exists("cached.org.", dns.TypeA, dns.ClassINET, false, false, c.bucketOf("10.240.0.1"))
			if item == nil {
			...
			if got := c.exists("cached.org.", dns.TypeA, dns.ClassINET, false, false, c.bucketOf("10.240.0.1")).Rcode; got != tc.expectedRcode {
	...
	for _, mode := range modes {
		for _, invalid := range invalidResponses {
			t.Run(mode.name+"/"+invalid.name, func(t *testing.T) {
				...
				serveStaleRecheckRequest(t, c, req)
				item := c.exists("cached.org.", dns.TypeA, dns.ClassINET, true, false, c.bucketOf("10.240.0.1"))
				if item == nil {
					t.Fatal("expected primed cache item")
				}
				clock.Set(2 * time.Second)
