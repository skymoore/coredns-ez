...
type Cache struct {
	...
	// Per-source cache keying. Responses are bucketed by the client's source
	// network so split-horizon answers (ACL views) never leak between scopes.
	// mask4/mask6 are the IPv4/IPv6 prefix lengths folded into the cache key.
	mask4 int
	mask6 int
...
// qname holds the already lowercased qname. src holds the masked source network of the
// client that triggered the lookup, so split-horizon answers are keyed per source scope.
func key(qname string, m *dns.Msg, t response.Type, do, cd bool, src []byte) (bool, uint64) {
	...
	return true, hash(qname, m.Question[0].Qtype, m.Question[0].Qclass, do, cd, src)
}

// bucketOf folds a client source address into key material: the address masked
// to the configured prefix, followed by the prefix length itself. Two clients
// inside one bucket share entries; clients across buckets never do. A nil or
// unparseable address collapses to the shared (empty) bucket.
func (c *Cache) bucketOf(remote string) []byte {
	...
	if v4 := ip.To4(); v4 != nil {
		m := min(c.mask4, 32)
		return append(v4.Mask(net.CIDRMask(m, 32)), byte(m))
	}
	m := min(c.mask6, 128)
	return append(ip.Mask(net.CIDRMask(m, 128)), byte(m))
}
...
func hash(qname string, qtype, qclass uint16, do, cd bool, src []byte) uint64 {
	h := fnv.New64()
...
type ResponseWriter struct {
	...
	*Cache
	...
	src []byte // masked source-network bucket of the original client (split-horizon keying)
...
func (w *ResponseWriter) WriteMsg(res *dns.Msg) error {
	...
	hasKey, key := key(w.state.Name(), res, mt, w.do, w.cd, w.src)
