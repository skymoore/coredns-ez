...
func TestTransferInCatalogPersistsMembers(t *testing.T) {
	...
	s := newTestSecondary(t, origin, z, true)
...
func newCatalogMigrationFixture(t *testing.T, oldID, newID, coo string, blockTarget bool) *catalogMigrationFixture {
	...
	}, fall.F{}, map[string]plugin.Zones{oldOrigin: nil, newOrigin: nil}, persistConfig{dir: t.TempDir()})
...
func waitForAnswer(t *testing.T, s *SecondaryPersist, name string, qtype uint16) *dns.Msg {
	t.Helper()
...
func waitForAddress(t *testing.T, s *SecondaryPersist, zone, address string) {
	t.Helper()
